...
- **Output:** your Windows **Downloads** folder, under `SEC Filings\{TICKER}\{FORM}\` (e.g. `C:\Users\<you>\Downloads\SEC Filings\MRVL\10-K\`). If you moved Downloads to another drive, that location is used. Change it with `--out`. Existing files are skipped unless `--overwrite`.
